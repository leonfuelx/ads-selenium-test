...
const getSessionData = async (arr, clientUrls) => {
...
const getNetworkData = async url => {
    ...
    if (scriptExecutionTime === 0) {
        return `No Pixel (sessionid: ${sessionId})`;
    } else if (fuelXCalls < 5) {
        return `Pixel: All Endpoints not called (${fuelXCalls}/7 calls), (sessionid: ${sessionId})`
    } else if (scriptExecutionTime > 5000) {
...
const sendMessage = async (body) => {
    let message = {
        ...
        to: ['leon.wang@fuelx.com', 'santosh@fuelx.com'],
...
api.initApi = async (clientUrls, testDate) => {
    date = testDate;
    ...
    await getSessionData(sessionIdArr, clientUrls);
}
